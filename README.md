...
### Running the Test Suite (Local run)
...
### Running the Test Suite in Blockcerts context, as a dependency

1. `npm i https://github.com/blockchain-certificates/vc-test-suite.git\#blockcerts-work --install-strategy=nested`
   `install-strategy=nested` will ensure the test suite will be able to access the node wrapper through the correct path
2. (optional, if needed) `vc-test-suite set-config [config option] [config value]`
   ie: `vc-test-suite set-config generatorOptions "--issuer_path ${PWD}"`
3.`vc-test-suite report:blockcerts`
